      ...
      <div className="hidden md:block bg-[#0A0A0A] text-white py-2">
        ...
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between h-20">
            ...
            >
              <span className="text-2xl font-bold text-[#0A0A0A]">ICONIC LUXURY SAFARIS</span>
            </motion.div>
              ...
              <div className="flex items-center space-x-8">
                {menuItems.map((item, index) => (
                  <motion.a
                    key={item.name}
                    href={item.href}
                    className="text-[#0A0A0A] hover:text-[#C6A870] transition-colors relative group"
                    whileHover={{ scale: 1.05 }}
                ...
                <motion.button
                  className="bg-[#C6A870] text-white px-6 py-2 hover:bg-[#C6A870]/90 transition-all duration-300 border border-[#C6A870] hover:shadow-[0_0_15px_rgba(198,168,112,0.5)]"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
